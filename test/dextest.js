...
contract("Dex", accounts => {
    ...
    it("should throw an error if token balance is too low when creating SELL limit order", async() => {
        ...
        let Dmed = await DMED.deployed()
        await truffleAssert.reverts(
            dex.createLimitOrder(1, web3.utils.fromUtf8("DMED"), 10, 1)
        )
        await Dmed.approve(dex.address, 500);
        await dex.addToken(web3.utils.fromUtf8("DMED"), Dmed.address, {from:accounts[0]})
        await dex.deposit(10, web3.utils.fromUtf8("DMED"));
        await truffleAssert.passes(
            dex.createLimitOrder(1, web3.utils.fromUtf8("DMED"), 10, 1)
        )
   ...
   //The BUY order book should be ordered on price from highest to lowest starting at index 0
    it("The BUY order book be ordered on price from highest to lowest starting at index 0", async() => {
        let dex = await Dex.deployed()
        let Dmed = await DMED.deployed()
        await Dmed.approve(dex.address, 500);
